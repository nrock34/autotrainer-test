...
def write_playlist(username: str, playlist_id: str, spotify):
...
def enable_multicore(reference_file, buffercores=1):
    native_cpu_count = multiprocessing.cpu_count() - buffercores
    count = 0
    with open(reference_file, "r", encoding='utf-8') as ff:
        for line in ff:
            count+=1
    return native_cpu_count if native_cpu_count <= count else count
...
def sp2mp3(client_id, client_secret, username, playlist_uri, isMain=False, hasName=None, cus_name=None):
    # Parameters
    ...
    if hasName is None or False:
        playlist_name = write_playlist(username, playlist_uri, spotify)
    else:
        playlist_name = cus_name
    ...
    multicore_support = enable_multicore(reference_file, buffercores=1)
